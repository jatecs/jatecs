...
/**
 * Implements a kernel-based cosine Distributional Correspondence Function.
 * @see IDistributionalCorrespondenceFunction
 * */
public class CosineDCF extends AVectorDCF{
	
	private boolean _normalized; 
	public CosineDCF(IIndex index){
	...
	protected double distributionalCorrespondence(IVector featDist, IVector pivotDist) {
		if(_normalized)
			return featDist.dotProduct(pivotDist);
		else
			return featDist.cosineSimilarity(pivotDist);
	}
	...
	protected double distributionalRandomCorrespondence(int feat1, int feat2) {
		...
		double randsim=Math.sqrt(prevf*prevp);
		return randsim;
	}

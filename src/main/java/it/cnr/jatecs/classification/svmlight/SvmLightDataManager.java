...
public class SvmLightDataManager implements IDataManager {

    protected SvmLightClassifierCustomizer _svmLightClassifierCustomizer;

    public SvmLightDataManager(
            SvmLightClassifierCustomizer svmLightClassifierCustomizer) {
        _svmLightClassifierCustomizer = svmLightClassifierCustomizer;
    }

    public void write(String modelDir, IClassifier classifier)
            throws IOException {
        SvmLightClassifier c = (SvmLightClassifier) classifier;

        File d = new File(modelDir);
        if (!d.exists())
            d.mkdirs();

        // Copy all generated classifier in the specified model dir.
        DataOutputStream os = new DataOutputStream(new FileOutputStream(
                modelDir + Os.pathSeparator() + "mapping.db"));
        ...
        for (int i = 0; i < cats.length; i++) {
            ...
            File src = new File(c.getClassifierDataDir() + Os.pathSeparator()
                    + relativePath + Os.pathSeparator() + "model.txt");
            File dest = new File(modelDir + Os.pathSeparator() + relativePath);
            if (!dest.exists())
                dest.mkdirs();
            dest = new File(modelDir + Os.pathSeparator() + relativePath
    ...
    public IClassifier read(String modelDir) throws IOException {
        SvmLightClassifier classifier = new SvmLightClassifier(modelDir);

        // Copy all generated classifier in the specified model dir.
        DataInputStream is = new DataInputStream(new FileInputStream(modelDir
                + Os.pathSeparator() + "mapping.db"));
        int length = is.readInt();

        classifier.setRuntimeCustomizer(_svmLightClassifierCustomizer);
    ...
    public void write(IStorageManager storageManager, String modelName,
        ...
        SvmLightClassifier c = (SvmLightClassifier) learningData;
    ...
    public IClassifier read(IStorageManager storageManager, String modelName) {
        try {
            ...
            SvmLightClassifier classifier = new SvmLightClassifier(
                    tempPath.toString());
    ...
    public IClassifierRuntimeCustomizer readClassifierRuntimeConfiguration(
            IStorageManager storageManager, String modelName) {
        return (SvmLightClassifierCustomizer) _svmLightClassifierCustomizer
                .cloneObject();
    }

...
public class Reuters21578Classifier extends JatecsModule {

    private IClassifier _classifier;

    private String _classifierDir;

    private String _validCategories;

    public Reuters21578Classifier(IIndex testIndex, IClassifier classifier, String classifierDir, String validCategories) {
        super(testIndex, Reuters21578Classifier.class.getName());
    ...
    protected void processModule() {
        ...
        TroveCategoryDBBuilder b = new TroveCategoryDBBuilder();
        FileCategoryReader catReader = new FileCategoryReader(_validCategories, b);
        TShortArrayList validCategories = new TShortArrayList();
        ICategoryDB catDB = catReader.getCategoryDB();
        IShortIterator catIt = catDB.getCategories();
        while (catIt.hasNext()) {
            short cat = catIt.next();
            String catName = catDB.getCategoryName(cat);
            short indexCat = index().getCategoryDB().getCategory(catName);
            if (indexCat >= 0)
                validCategories.add(indexCat);
        }

        ClassificationComparer cc = new ClassificationComparer(index().getClassificationDB(), builder.getClassificationDB(), new TShortArrayListIterator(validCategories));
        ContingencyTableSet set = cc.evaluate();

        String fname = _classifierDir;
        try {
            ContingencyTableDataManager.writeContingencyTableSet(fname, set);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        EvaluationReport.printReport(set, index().getCategoryDB());

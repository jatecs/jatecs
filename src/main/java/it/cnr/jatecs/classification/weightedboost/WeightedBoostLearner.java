...
import it.cnr.jatecs.indexing.discretization.FeatureWeightValuesDiscretizer;
...
public class WeightedBoostLearner extends BaseLearner {

    public WeightedBoostLearner() {
        _customizer = new WeightedBoostLearnerCustomizer();
    }

    public IClassifier build(IIndex trainingIndex) {

        WeightedBoostLearnerCustomizer customizer = (WeightedBoostLearnerCustomizer) _customizer;
        ...
        WeightedBoostClassifier classifier = new WeightedBoostClassifier();
        classifier._hypothesis = new InMemoryWeightedWeakHypothesis[customizer._maxNumIterations];
        classifier._maxNumIterations = customizer._maxNumIterations;
        WeightedBoostClassifierCustomizer abcust = (WeightedBoostClassifierCustomizer) classifier
        ...
        JatecsLogger.status().print("Discretize features...");
        FeatureWeightValuesDiscretizer discretizer = new FeatureWeightValuesDiscretizer(
                trainingIndex, customizer.getDiscretizer());
        TreeSet<DiscreteBin>[] bins = discretizer.discretizeWeights();
        classifier.bins = bins;
        JatecsLogger.status().println("done.");

        WeakLearnerParams params = new WeakLearnerParams();
        params.setBins(bins);
        ...
        for (int i = 0; i < customizer._maxNumIterations; i++) {
            ...
            IWeightedWeakHypothesis wh = null;
            ...
            updateDistributionMatrix(matrix, wh, trainingIndex, params);
    ...
    protected void obtainInitialDistributionMatrix(IWeighting3DManager matrix,
                                                   IIndex index) {
        if (((WeightedBoostLearnerCustomizer) _customizer)._matrixFileLoad != null) {
            obtainInitialDistributionMatrixFromStoredMatrix(matrix);
        } else {
            if (((WeightedBoostLearnerCustomizer) _customizer)._initDistType == InitialDistributionMatrixType.UNIFORM)
                obtainUniformDistributionMatrix(matrix, index);
            else if (((WeightedBoostLearnerCustomizer) _customizer)._initDistType == InitialDistributionMatrixType.CATEGORY_ORIENTED)
                obtainCategoryOrientedDistributionMatrix(matrix, index);
            else if (((WeightedBoostLearnerCustomizer) _customizer)._initDistType == InitialDistributionMatrixType.MAXIMIZE_F1)
                obtainMaximizeF1DistributionMatrix(matrix, index);
            else
                throw new RuntimeException(
                        "Type "
                                + ((WeightedBoostLearnerCustomizer) _customizer)._initDistType
    ...
    protected void obtainCategoryOrientedDistributionMatrix(
        ...
        float factorPositive = ((WeightedBoostLearnerCustomizer) _customizer)._factorPositive;
    ...
    protected void obtainMaximizeF1DistributionMatrix(
        ...
        if (((WeightedBoostLearnerCustomizer) _customizer)._perCategoryNormalization) {
    ...
    protected void updateDistributionMatrix(IWeighting3DManager matrix,
                                            IWeightedWeakHypothesis wh, IIndex index, WeakLearnerParams params) {
        WeightedBoostLearnerCustomizer customizer = (WeightedBoostLearnerCustomizer) _customizer;
        ...
        for (int document = 0; document < matrix.getSecondDimensionSize(); document++) {
            for (short category = 0; category < matrix.getFirstDimensionSize(); category++) {
                ...
                double value = 0;
                WeightedHypothesisData v = wh.value(category);
                int pivot = v.pivot;
                if (pivot >= 0) {
                    TreeSet<DiscreteBin> b = params.getBins()[pivot];
                    if (index.getContentDB()
                            .hasDocumentFeature(document, pivot)) {
                        double w = index.getWeightingDB()
                                .getDocumentFeatureWeight(document, pivot);
                        int valIndex = computeBinIndex(w, b);
                        value = v.c1ConstantValues[valIndex];
                    } else
                        value = v.c0;
                }
    ...
    public IClassifier mergeClassifiers(Vector<IClassifier> classifiers) {
        ...
        for (int i = 0; i < classifiers.size(); i++) {
            ...
            WeightedBoostClassifier cl = (WeightedBoostClassifier) c;
        ...
        WeightedBoostClassifier cl = new WeightedBoostClassifier();

        cl._hypothesis = new IWeightedWeakHypothesis[maxIterations];
        cl._maxNumIterations = maxIterations;
        WeightedBoostClassifierCustomizer abcust = (WeightedBoostClassifierCustomizer) cl
                .getRuntimeCustomizer();
        ...
        for (int i = 0; i < maxIterations; i++) {
            IWeightedWeakHypothesis hyp = new InMemoryWeightedWeakHypothesis(
                    classifiers.size());

            for (short j = 0; j < classifiers.size(); j++) {
                WeightedBoostClassifier c = (WeightedBoostClassifier) classifiers
                        .get(j);
                WeightedHypothesisData d = c._hypothesis[i].value((short) 0);
                hyp.setValue(j, d);
            }

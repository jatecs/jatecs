...
public class ModRegularizedLogisticRegressionLearner extends BaseLearner {

    public ModRegularizedLogisticRegressionLearner() {
        setRuntimeCustomizer(new ModRegularizedLogisticRegressionLearnerCustomizer());
    }

    protected double C(double r_i, double c_k) {
        if (r_i <= 1)
            return 2 * (r_i - 1);
        else
            return 2 * c_k * (r_i - 1);
    ...
    protected double F(double x, double y, double c_k) {
        if (x <= 1 + Math.abs(y))
            return 2;
        else
            return 2 * c_k;
    ...
    protected double compute_delta_vj(IIntIterator docs, int j, IIndex index,
                                      double c_k, double lambda, double[] weight, double[] delta,
                                      double[] r, short catID, double[] y, double[] weightsDoc) {
        ...
        while (docs.hasNext()) {
            int i = docs.next();
            numerator += (C(r[i], c_k) * weightsDoc[i] * y[i]);
            denominator += (F(r[i], delta[j] * weightsDoc[i], c_k)
                    * weightsDoc[i] * weightsDoc[i]);
        }
    ...
    public IClassifier build(IIndex trainingIndex) {
        ModRegularizedLogisticRegressionLearnerCustomizer cust = (ModRegularizedLogisticRegressionLearnerCustomizer) getRuntimeCustomizer();

        ModRegularizedLogisticRegressionClassifier cl = new ModRegularizedLogisticRegressionClassifier(
        ...
        while (cats.hasNext()) {
            ...
            for (int k = 0; k < cust.K; k++) {
                double c_k = cust.getCK(cust.K, k + 1);
                ...
                for (int j = 0; j < numFeats; j++) {
                    ...
                    double delta_vj = compute_delta_vj(docs, j, trainingIndex,
                            c_k, cust.lambda, w, delta, r, catID, y, weightsDoc);
			...
			/*
             * for (int j = 0; j < numFeats; j++) { IIntIterator docs =
			 * trainingIndex.getContentDB().getFeatureDocuments(j); while
			 * (docs.hasNext()) { int docID = docs.next(); weightsDoc[docID] =
			 * trainingIndex.getWeightingDB().getDocumentFeatureWeight(docID,
			 * j); }
			 * 
			 * for (int k = 0; k < cust.K; k++) { double c_k =
			 * cust.getCK(cust.K, k+1); //System.out.println("It "+k);
			 * 
			 * double delta_vj = compute_delta_vj(docs, j, trainingIndex, c_k,
			 * cust.lambda, w, delta, r, catID, y, weightsDoc); double delta_wj
			 * = compute_delta_wj(delta_vj, delta[j]);
			 * 
			 * docs.begin(); while(docs.hasNext()) { int i = docs.next(); r[i]
			 * += delta_wj*weightsDoc[i]*y[i]; }
			 * 
			 * w[j] += delta_wj; delta[j] = (2*Math.abs(delta_wj))+cust.epsilon;
			 * } }
			 */

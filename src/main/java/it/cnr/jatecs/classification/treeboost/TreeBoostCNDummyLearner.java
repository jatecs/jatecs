...
public class TreeBoostCNDummyLearner extends BaseLearner {
    ...
    public TreeBoostCNDummyLearner(ILearner learner) {
        assert (learner != null);
    ...
    public IClassifier build(IIndex trainingIndex) {
        ...
        constructClassifiersAt(Short.MIN_VALUE, trainingIndex, c, stat);

        double numAvgNeg = ((double) stat.numNegatives)
                / trainingIndex.getCategoryDB().getCategoriesCount();

        JatecsLogger.status()
                .println(
                        "The average number of negatives per category is: "
                                + numAvgNeg);
    ...
    public void constructClassifiersAt(short catID, IIndex training,
                                       TreeBoostClassifier classifiers, Statistics stat) {
        ...
        while (childs.hasNext()) {
            ...
            for (short i = 0; i < training.getCategoryDB()
            ...
            TIntArrayListIterator docs = _negativesChooser
                    .selectNegatives(training.getCategoryDB()
            ...
            stat.numNegatives += docsList.size();
			...
			/*
             * if (_tsr != null) { JatecsLogger.status().println("Now apply TSR
			 * to index."); _tsr.computeTSR(idx);
			 * JatecsLogger.status().println("Ok. TSR applied."); }
			 * 
			 * if (_weighting != null) { JatecsLogger.status().println("Now
			 * apply weighting to document features."); idx =
			 * _weighting.computeWeights(idx);
			 * JatecsLogger.status().println("Ok. Weighting operation done."); }
			 * 
			 * JatecsLogger.status().println("The number of valid features is
			 * "+idx.getFeaturesDB().getFeaturesCount(false)+".");
			 * 
			 * if(_customizer.getInternalCustomizer(catID, categoryID)!=null)
			 * _learner.setRuntimeCustomizer(_customizer.getInternalCustomizer(catID,
			 * categoryID));
			 *  // Construct the classifier for this level. IClassifier c =
			 * _learner.build(idx);
			 *  // Save classifier. cl.add(c);
			 * clLRC.add(_learner.getRuntimeCustomizer());
			 * 
			 * if (_learner.getRuntimeCustomizer() instanceof
			 * IOptimizatorRuntimeCustomizer) { IOptimizatorRuntimeCustomizer cu =
			 * (IOptimizatorRuntimeCustomizer) _learner.getRuntimeCustomizer();
			 * _lastSavedCustomizer.setInternalCustomizer(catID, categoryID,
			 * cu.getOptimizedLearnerRuntimeCustomizer()); } else
			 * _lastSavedCustomizer.setInternalCustomizer(catID, categoryID,
        ...
        while (childs.hasNext()) {
            ...
            constructClassifiersAt(curCatID, training, classifiers, stat);
        }
    }

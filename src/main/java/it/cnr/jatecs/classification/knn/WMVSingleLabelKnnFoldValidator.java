...
public class WMVSingleLabelKnnFoldValidator extends BaseLearner {
    ...
    protected WMVSingleLabelKnnCommitteeClassifier _classifier;
    protected float[][] _globalSimMatrix;

    public WMVSingleLabelKnnFoldValidator(WMVSingleLabelKnnCommitteeClassifier classifier) {
    ...
    protected static int findMinimumNumberOfPositives(IIndex index) {
        ...
        while (cats.hasNext()) {
            ...
            int docs = index.getClassificationDB().getCategoryDocumentsCount(catID);
            if (docs < minimum) {
                minimum = docs;
            }
        }
    ...
    protected OptimalConfiguration optimizeFor(WMVSingleLabelKnnCommitteeClassifier classifier,
                                               IIndex training, IIndex validation, int minimumK, int maximumK, int stepK) {

        SingleLabelKnnCommitteeClassifierCustomizer cust = (SingleLabelKnnCommitteeClassifierCustomizer) classifier.getRuntimeCustomizer().cloneObject();
        ...
        for (int currentK = minimumK; currentK <= maximumK; currentK += stepK) {
            ...
            SingleLabelKnnCommitteeClassifierCustomizer cu = (SingleLabelKnnCommitteeClassifierCustomizer) cust.cloneObject();

            cu.setNumSimilar(currentK);

            classifier.setRuntimeCustomizer(cu);

            ClassificationResult[] results = new ClassificationResult[validation.getDocumentDB().getDocumentsCount()];
            IIntIterator docs = validation.getDocumentDB().getDocuments();
            while (docs.hasNext()) {
                int docID = docs.next();

                // Classify validation documents using current configuration.
                ClassificationResult res = classifier.classify(validation, docID);
                results[docID] = res;
            }
            ...
            for (int j = 0; j < results.length; j++) {
                ...
                builder.setDocumentCategory(res.documentID, res.categoryID.get(0));
            }

            ClassificationComparer cc = new ClassificationComparer(builder.getClassificationDB(), validation.getClassificationDB());
	...
	/*public static Pair<IIndex, IIndex> splitIndex(int step, IIndex index, int numValidationSteps)
	{
		int numPositives = findMinimumNumberOfPositives(index);
		...
		IShortIterator cats = index.getCategoriesDB().getCategories();
		while (cats.hasNext())
		{
			short catID = cats.next();
			
			numPositives = index.getClassificationDB().getCategoryDocumentsCount(catID);
			
			int numPositivesInValidation = 0;
			int numPositivesInTraining = 0;

			if (numPositives < numSteps)
			{
			...
			{
				numPositivesInValidation = numPositives / numSteps;
    ...
    protected IClassifier buildNotTextual(IIndex trainingIndex) {
        ...
        SingleLabelKnnCommitteeClassifierCustomizer customizer = (SingleLabelKnnCommitteeClassifierCustomizer) _classifier.getRuntimeCustomizer();
        ...
        for (int i = 0; i < cust.getKFoldValidationSteps(); i++) {
            JatecsLogger.status().println("Doing validation " + (i + 1) + "/" + cust.getKFoldValidationSteps() + "...");

            // Split the index in two parts: a training and a valiudation set.
            Pair<IIndex, IIndex> indexes = splitIndex(i, trainingIndex, cust.getKFoldValidationSteps());
            if (indexes == null)
            ...
            // Optimize classifier.
            OptimalConfiguration res = optimizeFor(_classifier, indexes.getFirst(), indexes.getSecond(), 1, 33, 2);

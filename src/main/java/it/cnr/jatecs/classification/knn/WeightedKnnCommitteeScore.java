...
public class WeightedKnnCommitteeScore implements IKnnCommitteeScorer {


    public WeightedKnnCommitteeScore() {
    }
    ...
    public ClassificationResult computeScore(KnnCommitteeClassifier cl,
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            for (int j = 0; j < res.score.size(); j++) {
                ...
                double val = -1;
                if (res.score.get(j) >= cust.getClassifierRange(catID).border)
                    val = 1;

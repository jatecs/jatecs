...
public class CRDCSSingleLabelKnnFoldValidator extends BaseLearner {

    protected CRDCSSingleLabelKnnCommitteeClassifier _classifier;
    protected float[][] _globalSimMatrix;

    public CRDCSSingleLabelKnnFoldValidator(
            CRDCSSingleLabelKnnCommitteeClassifier classifier) {
    ...
    protected OptimalConfiguration optimizeFor(
            CRDCSSingleLabelKnnCommitteeClassifier classifier, IIndex training,
        ...
        for (int currentK = minimumK; currentK <= maximumK; currentK += stepK) {
            ...
            for (int j = 0; j < results.length; j++) {
                ...
                builder.setDocumentCategory(res.documentID,
                        res.categoryID.get(0));
            }

            ClassificationComparer cc = new ClassificationComparer(
                    builder.getClassificationDB(),
                    validation.getClassificationDB());
    ...
    protected IClassifier buildNotTextual(IIndex trainingIndex) {
        ...
        for (int i = 0; i < cust.getKFoldValidationSteps(); i++) {
            ...
            // Split the index in two parts: a training and a validation
            // set.
            ...
            // Optimize classifier.
            OptimalConfiguration res = optimizeFor(_classifier,
                    indexes.getFirst(), indexes.getSecond(), 1, 33, 2);

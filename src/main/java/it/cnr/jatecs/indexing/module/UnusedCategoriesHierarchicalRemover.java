...
public class UnusedCategoriesHierarchicalRemover extends JatecsModule {
    ...
    public UnusedCategoriesHierarchicalRemover(IIndex index) {
    ...
    protected void processModule() {
        ...
        IClassificationDB classificationDB = source.getClassificationDB()
                .cloneDB(categoriesDB, source.getDocumentDB());
        IDomainDB domainDB = source.getDomainDB().cloneDB(categoriesDB,
                source.getFeatureDB());
        _index = new GenericIndex(source.getFeatureDB(), source
                .getDocumentDB(), categoriesDB, domainDB, source
                .getContentDB(), source.getWeightingDB(), classificationDB);
        _usedCategories = computeUsedCategories(_index);
        FilteredShortIterator unusedCategories = new FilteredShortIterator(
                categoriesDB.getCategories(), new TShortArrayListIterator(
                _usedCategories), true);
        ...
        for (short i = 0; i < _usedCategories.size(); ++i) {
            ...
            JatecsLogger.status().println(
                    source.getCategoryDB().getCategoryName(usedCategory)
                            + " (" + usedCategory + " -> " + i + ")");
    ...
    public IShortIterator getUnusedCategories() {
        return new FilteredShortIterator(index().getCategoryDB()
                .getCategories(), new TShortArrayListIterator(_usedCategories),
                true);
    }
    ...
    private TShortArrayList computeUsedCategories(IIndex index) {
        ...
        while (categories.hasNext()) {
            ...
            if (!categoriesDB.hasChildCategories(category)) {
                if (classificationDB.getCategoryDocumentsCount(category) != 0)

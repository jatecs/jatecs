...
public class SvmLightOutput {
    ...
    private static void writeIndex(String filename, IIndex index)
        ...
        TextualProgressBar bar = new TextualProgressBar(
                "Writing index in SVMLight format");

        int numItems = index.getDocumentDB().getDocumentsCount() * index.getCategoryDB().getCategoriesCount();
        int numAnalyzed = 0;
        ...
        while (catit.hasNext()) {
            ...
            BufferedWriter writer = new BufferedWriter(new FileWriter(filename + catId));

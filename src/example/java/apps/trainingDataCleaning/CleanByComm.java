...
public class CleanByComm {

    public static void main(String[] args) throws IOException {
        if (args.length != 2) {
            System.err
                    .println("Usage: CleanByComm <indexDirectory> <classifierDirectory>");
            return;
        }
        ...
        storageManager.open();
        AdaBoostClassifier classifier = (AdaBoostClassifier) dataManager.read(
        ...
        while (docs.hasNext()) {
            ...
            ClassificationResult crClassification = classifier.classify(index,
                    doc);
            for (int i = 0; i < crVariance.categoryID.size(); ++i) {
                short cat = crVariance.categoryID.getQuick(i);
        ...
        FileOutputStream fstream = new FileOutputStream(indexPath
                + Os.pathSeparator() + indexName + "_COMM-" + classifierName
                + ".txt");
